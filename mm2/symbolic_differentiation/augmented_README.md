...
- Add a simplification rule both implementations lack — say
  `(- $x $x) -> 0`, where repeating `$x` in the pattern makes the rule
  ...
  worked precedents, look at the `mulnL`/`mulnR` rules
  (`(* $x (neg 1)) -> (neg $x)`) — including how their names are chosen
  to sort after `mul0*`/`mul1*` so overlaps resolve the same way in
  both implementations — and the `negn` rule (`(neg (neg $x)) -> $x`).
