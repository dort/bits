...
def sc(metric = math.e) :
    density = math.log(metric-1.0)
    print(metric, density)
    return density



def random_tuple(len=3000, sparsity=0.5):

    result = tuple(random.choices([0, 1], weights=[1.0-sparsity, sparsity], k=len))
    
    return result


def multiple_RTs(num=10):
    result = [random_tuple(len=10, sparsity=sc(3 )) for _ in range(num)]
    return result
