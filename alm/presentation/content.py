"""Narrative, timing, and private speaker notes for the ALPC presentation."""

COMMIT = "8406e6a838442391fd3089958e1a2c1b44c57eda"
REPO = f"https://github.com/trueagi-io/FabricPC/blob/{COMMIT}/"
PAPER = "https://arxiv.org/abs/2605.31022v1"
EPC_PAPER = "https://arxiv.org/abs/2505.20137"

SOURCES = {
    "P": ("Seely & Gould (2026), supplied paper, v1", PAPER),
    "E": ("Goemaere et al., ePC paper", EPC_PAPER),
    "I": ("FabricPC: sPC inference and solver lifecycle", REPO + "fabricpc/core/inference.py"),
    "R": ("FabricPC: EPCInference implementation", REPO + "fabricpc/core/inference_epc.py"),
    "N": ("FabricPC: state types and pytree registration", REPO + "fabricpc/core/types.py"),
    "W": ("FabricPC: local weight-gradient path", REPO + "fabricpc/core/learning.py"),
    "G": ("FabricPC: training with ePC guide", REPO + "docs/user_guides/17_training_with_epc.md"),
    "D": ("FabricPC: ePC regime and stability report", REPO + "docs/reports/epc_regime_and_stability_report.md"),
    "T": ("FabricPC: ePC tests and equivalence caveats", REPO + "tests/test_inference_epc.py"),
}


def slide(title, subtitle, seconds, section, sources, talk, prep, transition=""):
    return dict(title=title, subtitle=subtitle, seconds=seconds, section=section,
                sources=sources, talk=talk.strip(), prep=prep.strip(), transition=transition)


SLIDES = [
slide("Augmented Lagrangian\nPredictive Coding", "From local prediction errors to backpropagation credit", 30, "THE QUESTION", ["P", "R"],
...
slide("First: what are we trying to learn?", "A forward computation and a loss define the learning problem.", 90, "FIRST PRINCIPLES", ["P"],
...
slide("Backpropagation assigns credit by the chain rule", "Each hidden layer receives the derivative of the downstream loss.", 90, "FIRST PRINCIPLES", ["P"],
...
slide("Standard PC lets hidden states move", "Inference minimizes a supervised loss plus layerwise prediction errors.", 90, "STANDARD PC", ["P", "I"],
...
slide("A finite penalty changes the equilibrium", "The hidden states can improve the loss by violating the forward equations.", 90, "STANDARD PC", ["P"],
...
slide("ePC optimizes prediction errors directly", "Same energy on an acyclic graph; a different route through it.", 90, "ERROR PC", ["E", "R", "T"],
...
slide("ePC can be BP-like before it equilibrates", "A quick learning signal and a converged PC solution are different endpoints.", 90, "ERROR PC", ["R", "G", "D"],
...
slide("Add a multiplier that remembers violations", "The augmented Lagrangian combines a task loss, constraints, and a penalty.", 90, "PC-ALM MECHANISM", ["P"],
...
slide("One batch: relax, accumulate, then learn", "Algorithm 1 uses T activity steps and T−1 multiplier updates.", 120, "PC-ALM MECHANISM", ["P"],
...
slide("Replace the residual credit with a composite", "Both activity inference and hidden-weight learning use c = λ + ρr.", 90, "PC-ALM MECHANISM", ["P", "W"],
...
slide("Why the endpoint gives the BP gradient", "Zero residuals restore forward states; nonzero multipliers retain credit.", 90, "PC-ALM MECHANISM", ["P"],
...
slide("A scalar example makes the difference visible", "Same weights and sample as before; only the inference rule changes.", 90, "WORKED EXAMPLE", ["P"],
...
slide("The paper observes faster credit propagation", "Local recurrence produces a traveling credit signal in deep networks.", 60, "REPORTED EVIDENCE", ["P"],
...
slide("The headline result: the deep–narrow gap closes", "Reported Fashion-MNIST results at a matched PC inference budget.", 90, "REPORTED EVIDENCE", ["P"],
...
slide("Promising mechanism; limited practical evidence", "Separate endpoint theory, finite-budget accuracy, and measured cost.", 90, "TRADEOFFS", ["P", "R"],
...
slide("Three mechanisms, three different tradeoffs", "Assume a matched acyclic graph, loss, penalty, initialization, and scaling.", 90, "COMPARISON", ["P", "I", "R", "T"],
...
slide("FabricPC needs a coordinated extension", "Proposed design based on the inspected source; no solver has been implemented here.", 90, "FABRICPC PROPOSAL", ["N", "I", "W", "R"],
...
slide("Evaluate correctness before model-scale claims", "A proposed experiment sequence with explicit comparisons and decision criteria.", 90, "FABRICPC PROPOSAL", ["P", "D", "T"],
...
slide("Recommendation: test PC-ALM as a research method", "The opportunity is local BP-consistent credit; the practical win must be measured.", 60, "TEAM DECISION", ["P", "R"],
...
slide("Discussion: what should the first experiment prove?", "Choose the primary objective before choosing the benchmark.", 180, "DISCUSSION", [],
...
slide("Notation and conventions", "Backup · one symbol, one meaning in this presentation", 0, "BACKUP A", ["P", "R"],
...
slide("Derive the scalar example by hand", "Backup · all endpoint values follow from two scalar derivatives", 0, "BACKUP B", ["P"],
...
slide("Why ePC changes dynamics but preserves the objective", "Backup · acyclic error coordinates induce a different state-space metric", 0, "BACKUP C", ["R", "T", "D"],
...
slide("Linear stability is a property of the joint update", "Backup · the simple bound omits the supervised-loss curvature block", 0, "BACKUP D", ["P"],
...
slide("Integration details that affect the algorithm", "Backup · objective semantics and update order are part of correctness", 0, "BACKUP E", ["N", "I", "W", "T"],
...
slide("Sources and reproducibility", "Backup · paper claims, repository observations, and proposals are kept distinct", 0, "BACKUP F", ["P", "E", "R", "I", "N", "W", "G", "D", "T"],
...
assert sum(s["seconds"] for s in SLIDES) == 1800
